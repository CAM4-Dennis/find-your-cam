...
// Save a copy of the original SPA shell before pre-rendering overwrites index.html
copyFileSync(join(DIST, "index.html"), join(DIST, "_app.html"));

// Load niche content data (generated by export-niche-content.mjs)
let nicheContentCache = {};
try {
  nicheContentCache = JSON.parse(readFileSync(join(__dirname, ".niche-content-cache.json"), "utf-8"));
} catch { /* no content cache available — body will use fallback */ }

// Load combo content data (generated by export-combo-content.mjs)
...
// ============================================================
// Gender × Category combo pages (NL + EN only)
// ============================================================

console.log("\n👫 Pre-rendering gender × category pages...");
...
console.log(`✅ Pre-rendered ${comboCount} gender × category pages`);

// ============================================================
// Platform × Category combo pages (NL + EN only)
// ============================================================

console.log("\n🖥️  Pre-rendering platform × category pages...");

let platCatCount = 0;

for (const [comboKey, langData] of Object.entries(comboContentCache.platformCategory)) {
  // comboKey = "chaturbate/teen"
...
console.log(`✅ Pre-rendered ${platCatCount} platform × category pages`);

// ============================================================
// Language-speaking pages (NL + EN only)
// ============================================================

console.log("\n🗣️  Pre-rendering language-speaking pages...");

let langSpeakCount = 0;
...
console.log(`✅ Pre-rendered ${langSpeakCount} language-speaking pages`);
...
function getNicheContentForPrerender(slug, nicheName, lang) {
  const override = nicheContentCache[slug];
  if (override?.[lang]) return override[lang];
  if (override?.nl && lang !== "en") return override.nl;
  // Template fallback
  if (lang === "en") {
    return {
      intro: `Explore the best ${nicheName.toLowerCase()} videos from cam models on CAM4. This niche features a wide variety of content from real amateur performers.`,
      content: `The **${nicheName.toLowerCase()} videos** on CAM4 showcase authentic content from real cam models. From short clips to extended sessions, there is something for every taste. All content is created by verified amateur performers who share their most intimate moments.\n\nCAM4 stands out for its **community-driven approach**. Models interact directly with viewers and regularly upload new video content. Whether you prefer solo shows or couples content, the ${nicheName.toLowerCase()} niche has it all.`,
      faq: [
        { q: `Are ${nicheName.toLowerCase()} videos free to watch?`, a: `Yes, you can watch free preview videos on CAM4. Create a free account for full access to all ${nicheName.toLowerCase()} content.` },
        { q: `Can I watch live ${nicheName.toLowerCase()} shows?`, a: `Absolutely! Many models in the ${nicheName.toLowerCase()} niche are live daily. You can watch for free and interact via chat.` },
        { q: `How do I find the best ${nicheName.toLowerCase()} cam models?`, a: `Use the filters on the niche page to sort by popularity, newest, or most viewed. You can also search for specific attributes.` },
      ],
      faqTitle: `Frequently asked questions about ${nicheName.toLowerCase()} videos`,
    };
  }
  return {
    intro: `Ontdek de beste ${nicheName.toLowerCase()} video's van cam modellen op CAM4. Deze niche biedt een grote variatie aan content van echte amateur performers.`,
    content: `De **${nicheName.toLowerCase()} video's** op CAM4 tonen authentieke content van echte cam modellen. Van korte clips tot uitgebreide sessies, er is voor ieder wat wils. Alle content wordt gemaakt door geverifieerde amateur performers die hun meest intieme momenten delen.\n\nCAM4 onderscheidt zich door de **community-gedreven aanpak**. Modellen hebben direct contact met kijkers en uploaden regelmatig nieuwe video content. Of je nu de voorkeur geeft aan solo shows of koppels content, de ${nicheName.toLowerCase()} niche heeft het allemaal.`,
    faq: [
      { q: `Zijn de ${nicheName.toLowerCase()} video's gratis te bekijken?`, a: `Ja, op CAM4 kun je gratis preview video's bekijken. Maak een gratis account aan voor volledige toegang tot alle ${nicheName.toLowerCase()} content.` },
      { q: `Kan ik ook live ${nicheName.toLowerCase()} shows bekijken?`, a: `Absoluut! Veel modellen in de ${nicheName.toLowerCase()} niche zijn dagelijks live. Je kunt gratis meekijken en via de chat communiceren.` },
      { q: `Hoe vind ik de beste ${nicheName.toLowerCase()} cam modellen?`, a: `Gebruik de filters op de niche-pagina om te sorteren op populariteit, nieuwste of meest bekeken.` },
    ],
...
function buildNicheBodyHtml(lang, slug, nicheName, seoDesc) {
  const nicheContent = getNicheContentForPrerender(slug, nicheName, lang);
  const navLinks = {
  ...
  // Build FAQ HTML
  let faqHtml = "";
  if (nicheContent.faq && nicheContent.faq.length > 0) {
    const faqItems = nicheContent.faq.map(f =>
      `<details><summary>${escapeHtml(f.q)}</summary><p>${escapeHtml(f.a)}</p></details>`
    ).join("");
    faqHtml = `<section><h2>${escapeHtml(nicheContent.faqTitle)}</h2>${faqItems}</section>`;
  }

  // Build content section
  let contentHtml = "";
  if (nicheContent.content) {
    contentHtml = `<section>${renderMarkdownContent(nicheContent.content)}</section>`;
  }

  // Breadcrumb
  const videosLabel = lang === "nl" ? "Niche Video's" : "Niche Videos";
  const prefix = langPrefixes[lang] || "";
  const breadcrumb = `<nav aria-label="breadcrumb"><a href="${prefix || "/"}">StartVagina</a> &gt; <a href="${prefix}/videos">${escapeHtml(videosLabel)}</a> &gt; <span>${escapeHtml(nicheName)}</span></nav>`;

  return `<div id="root"><nav aria-label="Main">${navLinks[lang] || navLinks.nl}</nav>${breadcrumb}<main><h1>${h1}</h1><p>${escapeHtml(nicheContent.intro || seoDesc)}</p>${contentHtml}${faqHtml}</main><footer><p>\u00a9 ${new Date().getFullYear()} StartVagina.nl</p></footer></div>`;
}
